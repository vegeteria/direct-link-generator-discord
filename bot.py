# ==============================================================================
#                 Direct Link Generator - Discord Bot
#
# Description:
#   A self-contained Discord bot that generates direct download links from
#   various file hosting services. This version supports loading keys
#   from a .env file. This bot does not use any AI.
#
# ==============================================================================
...
# Third-party imports - install these via pip
import requests
...
# --- Custom Exceptions and Constants from User's File ---

class DirectDownloadLinkException(Exception):
    """Custom exception for direct link generation errors."""
    pass

PASSWORD_ERROR_MESSAGE = "This link is password protected. Please send the link in the format: link::password"
user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:122.0) Gecko/20100101 Firefox/122.0"

# --- Utility Functions (Re-implemented from user's project structure) ---

def speed_string_to_bytes(size_str: str) -> int:
    """Converts a size string (e.g., '10 MB', '2.5 GB') to bytes."""
    size_str = size_str.strip()
    size_data = findall(r'(\d+(?:\.\d+)?)\s*([KMGTP]?B)', size_str.upper())
    if not size_data:
        return 0
    size_val, size_unit = size_data[0]
    size_val = float(size_val)
    unit_map = {'B': 1, 'KB': 1024, 'MB': 1024**2, 'GB': 1024**3, 'TB': 1024**4, 'PB': 1024**5}
    return int(size_val * unit_map.get(size_unit, 0))

def is_share_link(url: str) -> bool:
    """
    Placeholder function. In the original project, this likely identified
    a specific category of sharer links. Defaulting to False to ensure
    only explicitly defined domain handlers are used.
    """
    if any(x in urlparse(url).hostname for x in ["sharer.pw", "shareus.io"]):
        return True
    return False

# ==============================================================================
#          ALL LINK GENERATION FUNCTIONS FROM functions_of_python.txt
# ==============================================================================
...
def buzzheavier(url):
    ...
    """
    logger.info(f"Starting buzzheavier for URL: {url}")
    pattern = r"^https?://buzzheavier\.com/[a-zA-Z0-9]+/?$"
    if not match(pattern, url):
        logger.warning(f"URL does not match buzzheavier pattern: {url}")
        return url

    def _bhscraper(url, folder=False):
        logger.info(f"Executing _bhscraper for URL: {url}")
        ...
        logger.info(f"Requesting with headers: {headers}")
        try:
            response = session.get(url)
            logger.info(f"Scraper response status: {response.status_code}")
            logger.info(f"Scraper response headers: {response.headers}")
            d_url = response.headers.get("Hx-Redirect")
            if not d_url:
                logger.error("Hx-Redirect header not found.")
                if not folder:
                    raise DirectDownloadLinkException(f"ERROR: Gagal mendapatkan data")
                return
            logger.info(f"Found Hx-Redirect URL: {d_url}")
            return d_url
        except Exception as e:
            logger.error(f"Exception in _bhscraper: {e}", exc_info=True)
            raise DirectDownloadLinkException(f"ERROR: {str(e)}") from e

    with requests.Session() as session:
        logger.info(f"Fetching initial page content for: {url}")
        response = session.get(url)
        logger.info(f"Initial page status: {response.status_code}")
        tree = HTML(response.text)
        
        hx_get_link = tree.xpath(
            "//a[contains(@class, 'link-button') and contains(@class, 'gay-button')]/@hx-get"
        )
        if hx_get_link:
            logger.info(f"Found hx-get link: {hx_get_link[0]}")
            return _bhscraper("https://buzzheavier.com" + hx_get_link[0])
        
        logger.info("No hx-get link found, checking for folders.")
        folders = tree.xpath("//tbody[@id='tbody']/tr")
        if folders:
            ...
            logger.error("No download link or folder structure found.")
...
def fuckingfast_dl(url):
    ...
    try:
        ...
        match_obj = search(pattern, content)
...
def uploadhaven(url):
    ...
    try:
        ...
        sleep(15)
        res = requests.post(url, data=data, headers={"Referer": url}, cookies=res.cookies)
        html = HTML(res.text)
...
def mediafile(url):
    ...
    try:
        ...
        sleep(60)
        res = requests.get(download_url, headers={"Referer": url}, cookies=res.cookies)
...
def hxfile(url):
    if not os.path.isfile("hxfile.txt"):
...
def pixeldrain(url):
    try:
        ...
        response = requests.get("https://pd.cybar.xyz/", allow_redirects=True)
...
def terabox(url):
    ...
    if "✅ Status" in req:
        ...
        raise DirectDownloadLinkException(f"ERROR: {req.get('message', 'File not found!')}")


def filepress(url):
    try:
        url = requests.get(f"https://filebee.xyz/file/{url.split('/')[-1]}").url
        raw = urlparse(url)
        json_data = {
            "id": raw.path.split("/")[-1],
    ...
    if "data" not in res2:
        raise DirectDownloadLinkException(f'ERROR: {res2["statusText"]}')
    json_data2 = {
        "id": res2["data"],
        "method": "publicDownlaod",
    }
    ...
    def __get_content(folderKey, folderPath="", content_type="folders"):
        ...
        if content_type == "folders":
            ...
            for folder in folders:
                if folderPath:
                    newFolderPath = os.path.join(folderPath, folder["name"])
                else:
                    newFolderPath = os.path.join(folder["name"])
            ...
            for file in files:
                ...
                if not folderPath:
                    folderPath = details["title"]
                item["path"] = os.path.join(folderPath)
                item["url"] = _url
                if "size" in file:
                    size = file["size"]
                    if isinstance(size, str) and size.isdigit():
...
def send_cm(url):
    ...
    def __writeContents(html_text, folderPath=""):
        ...
        for folder in folders:
            ...
            __writeContents(_html, os.path.join(folderPath, folder["folder_name"]))
...
def filelions_and_streamwish(url):
    ...
    if any(
        ...
        apiKey = os.getenv("STREAMWISH_API")
...
async def help_command(ctx):
    """Sends a help message with a list of supported sites."""
    supported_sites = [
        "1Fichier", "Akmfiles", "BerkasDrive", "Buzzheavier", "DevUploads", 
        "DoodStream", "EasyUpload", "FileLions", "FilePress", "FuckingFast", 
        "GitHub", "GoFile", "HxFile", "KrakenFiles", "LinkBox", "LulaCloud", 
        "MediaFile.cc", "MediaFire", "MP4Upload", "OSDN", "OneDrive", 
        "PixelDrain", "Racaty", "Send.cm", "Sharer", "Shrdsk", "SolidFiles", 
        "StreamHub", "StreamTape", "StreamVid", "StreamWish", "SwissTransfer", 
        "TeraBox", "TmpSend", "Upload.ee", "UploadHaven", "WeTransfer", 
        "Yandex Disk", "pCloud", "Qiwi.gg"
    ]
    await ctx.send(
        "I can generate direct links from the following services:\n\n"
...
async def on_message(message):
    """Handles incoming messages, processes links, and sends back the direct link."""
    if message.author == bot.user:
        return

    # Avoid processing commands as links
    if message.content.startswith(bot.command_prefix):
        await bot.process_commands(message)
        return

    # Simple check to see if the message content is a URL
    if "http" in message.content:
        link = message.content.strip()
        try:
            # Let the user know the bot is working on the link
            processing_message = await message.channel.send(f"⏳ Processing link: {link}")

            direct_link = direct_link_generator(link)

            # Delete the "Processing" message
            await processing_message.delete()

            if isinstance(direct_link, dict):
                # Handle folder links (multiple files)
                response = f"**Folder: {direct_link['title']}**\n\n"
                for content in direct_link['contents']:
                    response += f"**{content['filename']}**: {content['url']}\n"
                await message.channel.send(response)
            else:
                # Handle single file links
                await message.channel.send(f"🔗 **Direct Link:** {direct_link}")
